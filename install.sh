echo "### Installing... ###"
path="foryou-Fast_move"
pakage_check=$(rpm -qa |grep unzip)
if [ "$pakage_check" = "" ]; then
  yum install unzip -y 1>/dev/null 2>/dev/null
fi
dir_check=$(ls "$path" 2>/dev/null)
if [ "$zip_file_check" = "" ]; then
  ...
  tar_file="$(ls "$path/" 2>/dev/null |grep tar)"
...
if ! grep -xq "alias q='source $path1/c ..'" ~/.bashrc; then
  error_check6=0
else
  error_check6=1
fi
...
if [ $error_check1 = 0 ] && [ $error_check2 = 0 ] && [ $error_check3 = 0 ] && [ $error_check4 = 0 ] && [ $error_check5 = 0 ] && [ $error_check6 = 0 ] && [ $error_check7 = 0 ] && [ $error_check8 = 0 ] && [ $error_check9 = 0 ] && [ $error_check10 = 0 ] && [ $error_check11 = 0 ] && [ $error_check12 = 0 ]; then
  mkdir -p $path2
  chmod 777 $path2
